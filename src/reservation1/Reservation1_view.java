...
extends Frame implements ActionListener,WindowListener,KeyListener{
    ...
    ChoiseFacility choiceFacility;    // 施設選択用選択ボックス
    TextField tfYear, tfMonth, tfDay;    //年月日のテキストフィールド
    TextArea textMessage;    // 結果表示用メッセージ欄

        public Reservation1_view(Reservtion1_control rcontrol) {
     ...
     // ボタンの生成
        ...
        // 設備チョイスボックスの生成
        choiceFacility = new ChoiseFacility();
       ...
       // 上部パネルの中央パネルに 施設[施設名選択]チョイス[概要説明]ボタンを追加
        ...
        buttonExplanation.addActionListener(this);             buttonVacancy.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if ( e.getSource() == buttonVacancy){ // 空き状況確認ボタン
        	...
        	result = rcontrol.gaiyou(); 
        }else if(e.getSource()==buttonCancel){ 
            result = rcontrol.cancel(); 
        }else if(e.getSource()==buttonConfirm){ 
            result=rcontrol.kakunin(); 
   		} 
