...
public class Reservtion1_control {
     MySQL mysql;
     Statement sqlStmt;
     String reservation_userid;
     private boolean flagLogin;

     public Reservtion1_control() {
        this.mysql = new MySQL();
        flagLogin = false;
    }

       public String loginLogout( Reservation1_view frame){
        String res=""; //結果を入れる変数
   ...
   //指定した日,施設の 空き状況(というか予約状況)
       ...
       public String makeReservation(Reservation1_view frame){
        String res="";        //結果を入れる変数
       ...
       public String kakunin(){ 
    	   ...
    	   if(flagLogin){ 
    	       try{ 
    	           ResultSet rs = mysql.kakunin(reservation_userid); 
    	           boolean exist = false; 
    	           while(rs.next()){ 
    	   	...
    	   	public String cancel(){
    	        ...
    	        if(flagLogin){  
    	 	        try { 
    	 	        	res = kakunin(); 
    	                int rs = mysql.cancel(reservation_userid); 
    	            if(rs != 0){ 
    	            	res += "予約を消しました"; 
    	            }
    	 	        }catch (Exception e) { 
